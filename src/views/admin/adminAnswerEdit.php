...
if (!empty($_POST['maCauTraLoiT'])) {
    $_SESSION['maCauTraLoi'] = $_POST['maCauTraLoiT'];
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!empty($_POST['noiDungTraLoi']) && !empty($_POST['noiDungSauTraLoi'])) {
        ...
        $query = 'call UPDATECAUTRALOI(?, ?, ?, ?)';

        try {
            $sth = $pdo->prepare($query);
            $sth->execute([
                $_SESSION['maCauTraLoi'],
                $_POST['noiDungTraLoi'],
...
<head>
    ...
    <title>Chỉnh sửa câu trả lời - Hội Học thuật</title>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="page-content">
                    <h4 class="mb-3">Sửa câu trả lời</h4>
                    <!-- ***** Banner Start ***** -->
                    <div class="row">
                        <div class="col-lg-12">
                                    ...
                                    <div class="col-lg-12 d-flex justify-content-center">
                                        <form action="#" method="post" class="w-100">
                                            <div class="mb-3">
                                                ...
                                                <textarea class="form-control" name="noiDungTraLoi" maxlength="256" placeholder="Nhập nội dung câu trả lời"><?php echo $_POST['noiDungTraLoiT']; ?></textarea>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label text-light">Nội dung hiển thị sau khi chọn đáp án</label>
                                                <textarea class="form-control" name="noiDungSauTraLoi" maxlength="256" placeholder="Nhập nội dung hiển thị sau khi chọn đáp án này, nội dung này sẽ được hiển thị ngay sau khi đáp án được chọn"><?php echo $_POST['noiDungSauTraLoiT']; ?></textarea>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label text-light">Đây là câu trả lời đúng? </label>
                                                <input type="checkbox" class="form-check-input" name="isCorrect" <?php if ($_POST['cauTraLoiDungT'] == 1) {
                                                                                                                        echo 'checked';
                                                                                                                    }  ?>>
                                            </div>
